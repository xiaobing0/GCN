...
list3 = [388, 788]
list4 = [56, 1006]
list5 = list(set(node_list) - set(list1) - set(list2) - set(list3) - set(list4))
...
#
list_common = list(dic1.values()) + list(dic2.values()) + list(dic3.values()) + list(dic4.values()) + list(dic5.values())  # 总的需要更新的点，即server上需要保存的点，序号为原始图中点的序号
...
for epoch in range(500):
    ...
    for key, value in p5.items():  # p1等于两个字典平均
        p1[key] = p1[key]*(len(list1)/2708) + p2[key]*(len(list2)/2708) + p3[key]*(len(list3)/2708) + p4[key]*(len(list4)/2708) + value*(len(list5)/2708)
    ...
    for key, value in p5.items():  # p1等于两个字典平均
        p1[key] = p1[key] * (len(list1) / 2708) + p2[key] * (len(list2) / 2708) + p3[key] * (len(list3) / 2708) + p4[
            key] * (len(list4) / 2708) + value * (len(list5) / 2708)
